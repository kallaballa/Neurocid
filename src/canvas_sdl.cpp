...
SDLCanvas::SDLCanvas(Coord width, Coord height) :
		screen_(NULL),
		width_(width),
		height_(height),
		scale_(1),
...
Sint16 SDLCanvas::scaleX(const Coord& c) {
	Coord preScale = 0.9;
...
Sint16 SDLCanvas::scaleY(const Coord& c) {
	Coord preScale = 0.9;
...
void SDLCanvas::drawText(const string& s, Coord x0, Coord y0, Color c) {
	SDL_Surface *text;
...
void SDLCanvas::updateOSD(const string& key, const string& value) {
	osdMap_[key] = value;
}

void SDLCanvas::renderOSD() {
	Coord y = 20;
	for(auto it : osdMap_) {
		SDL_Surface *text;
...
Coord SDLCanvas::renderText(const string& s, Coord y, Color c, bool left) {
	SDL_Surface *text;
	SDL_Color text_color = { c.r, c.g, c.b };
	text = TTF_RenderText_Solid(font_, s.c_str(), text_color);
	assert(text != NULL);
...
void SDLCanvas::renderTackerInfo() {
	Coord y = 20;
...
void SDLCanvas::drawTank(Tank& tank, Color c) {
...
void SDLCanvas::drawExplosion(Object& o, Color& c) {
