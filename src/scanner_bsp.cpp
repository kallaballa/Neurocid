...
void BspScanner::teamScan(Population& active, Population& passive, Bsp& bspFriends, Bsp& bspEnemies, BattleFieldLayout& bfl) {
...
void BspScanner::scan(BattleField& field) {
	assert(field.teams_.size() == 2);
	buildBsps(field);
	Population& teamA = field.teams_[0];
	Population& teamB = field.teams_[1];
