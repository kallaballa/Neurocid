...
void BrainFann::makeNN() {
	...
	layerArray[layout_.numLayers_ - 1] = layout_.numBrains_;

	nn_ = new fann*[layout_.numBrains_ + 1];
	nn_[0] = fann_create_standard_array(layout_.numLayers_, layerArray);
...
void BrainFann::run() {
	fann_type* outputs = fann_run(nn_[0], inputs_);
	fann_type val = -1;
	size_t selected = 1;
