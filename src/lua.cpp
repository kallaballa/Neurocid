...
public:
  ...
  void make_ship_fields(const Ship& ship) {
    ...
    make_field("defensiveHits", ship.defensiveHits_);
    make_field("offensiveHits", ship.offensiveHits_);
  ...
  void make_facility_layout(const FacilityLayout& fl) {
    ...
    make_field("maxDamage", fl.maxDamage_);
  ...
  void make_facility(const string& name, const Facility& f) {
    lua_newtable(L_);
...
double run_fitness_function(const string& name, Ship& ship, Facility& f, const BattleFieldLayout& bfl, bool winner) {
  ...
  loader->make_field("winner", winner);
  loader->make_facility("facility", f);
  lua_setglobal(L, "nc");
