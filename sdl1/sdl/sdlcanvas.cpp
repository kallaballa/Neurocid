...
SDLCanvas::SDLCanvas(Coord width, Coord height, BattleFieldLayout& bfl) :
		Canvas(), screen_(NULL), width_(width), height_(height), scale_(1), zoom_(
...
void SDLCanvas::drawStar(Star& s) {
	...
	for (size_t i = 0; i < s.radius; i++) {
		if (s.z_ == 0)
			circleRGBA(screen_, Sint16(s.x), Sint16(s.y), i, s.r, s.g, s.b,
					round(alpha));
		else {
			double r = i * 1.5 / zoom_ / 100;
			if (r != lastR) {
				auto scaled = scale(Vector2D(s.x, s.y), s.z_);
				circleRGBA(screen_, scaled.first, scaled.second, r, s.r, s.g,
...
void SDLCanvas::drawExplosion(Explosion& expl) {
	...
	for (size_t i = expl.tick_; i > 0; --i) {
		...
		if (lastR != r) {
			...
			circleRGBA(screen_, scaled.first, scaled.second, round(r * scale_),
					c.r, c.g, c.b, c.a);
		}
		lastR = r;
	}
...
void SDLCanvas::drawTrail(const Trail& trail, const Color& c) {
	...
	for (const Vector2D& v : trail) {
		...
		filledCircleRGBA(screen_, scaled.first, scaled.second, r, c.r, c.g, c.b,
				c.a);

		if (lastX != -1 && lastY != -1) {
			lineRGBA(screen_, lastX, lastY, scaled.first, scaled.second, c.r,
					c.g, c.b, 48);
		}
...
void SDLCanvas::drawEllipse(Vector2D loc, Coord rangeX, Coord rangeY, Color c) {
	...
	ellipseRGBA(screen_, scaled.first, scaled.second, round(rangeX * scale_),
			round(rangeY * scale_), c.r, c.g, c.b, c.a);
}

void SDLCanvas::fillCircle(Vector2D loc, Coord radius, Color c) {
#ifndef _NO_SDLGFX
	auto scaled = transform(loc);
	filledCircleRGBA(screen_, scaled.first, scaled.second,
			round(radius * scale_), c.r, c.g, c.b, c.a);
#endif
}

void SDLCanvas::drawLine(Coord x0, Coord y0, Coord x1, Coord y1, Color c,
		Coord s) {
	auto scaled0 = transform(Vector2D(x0, y0), s);
	auto scaled1 = transform(Vector2D(x1, y1), s);
	lineRGBA(screen_, scaled0.first, scaled0.second, scaled1.first,
			scaled1.second, c.r, c.g, c.b, c.a);
}

void SDLCanvas::drawRect(Coord x0, Coord y0, Coord x1, Coord y1, Color c,
		Coord s) {
	auto scaled0 = transform(Vector2D(x0, y0), s);
	auto scaled1 = transform(Vector2D(x1, y1), s);
	rectangleRGBA(screen_, scaled0.first, scaled0.second, scaled1.first,
			scaled1.second, c.r, c.g, c.b, c.a);
}

void SDLCanvas::fillRect(Coord x0, Coord y0, Coord x1, Coord y1, Color c,
		Coord s) {
	auto scaled0 = transform(Vector2D(x0, y0), s);
	auto scaled1 = transform(Vector2D(x1, y1), s);
...
void SDLCanvas::drawFacility(Facility& facility, Color c) {
	...
	filledCircleRGBA(screen_, scaled.first, scaled.second,
			round((facility.layout_.radius_ / 100) * scale_), c.r, c.g, c.b,
			128);
	...
	for (size_t i = 0; i < 3; ++i) {
		...
		pieRGBA(screen_, scaled.first, scaled.second,
				round(scale_ * (facility.layout_.radius_ / 150)), start, end, 0,
				0, 255, 255);
		start = iRand(0, 360);
		end = iRand(start, 360);
		pieRGBA(screen_, scaled.first, scaled.second,
				round(scale_ * (facility.layout_.radius_ / 150)), start, end,
				255, 255, 0, 255);
	}
